...
export class InputHandler {
  ...
  handleRawInput(rawInput, value) {
    console.log(`[InputHandler] handleRawInput: rawInput=${rawInput}, value=${value}`); // ADDED
    if (!this.activeScheme) {
      console.log("[InputHandler] No active scheme."); // ADDED
      return;
    }

    const schemeActions = this.activeScheme;
    let gameActionNameFound = null; // ADDED for logging

    for (const gameActionName in schemeActions) {
      if (Object.prototype.hasOwnProperty.call(schemeActions, gameActionName)) {
        const mappedRawKeys = schemeActions[gameActionName];
        if (Array.isArray(mappedRawKeys) && mappedRawKeys.includes(rawInput)) {
          gameActionNameFound = gameActionName; // ADDED for logging
          console.log(`[InputHandler] Raw key '${rawInput}' maps to gameActionName: '${gameActionName}'`); // ADDED
          ...
          if (currentSchemeName) {
            if (!this.mappers[currentSchemeName]) {
              console.log(`[InputHandler] Creating new InputMapper for scheme: ${currentSchemeName}`); // ADDED
              this.mappers[currentSchemeName] = new InputMapper();
            }
            const mapper = this.mappers[currentSchemeName];
            if (mapper) {
              console.log(`[InputHandler] Calling mapper.mapInput for action: '${gameActionName}' with value: ${value}`); // ADDED
    ...
    if (!gameActionNameFound) { // ADDED for logging
      console.log(`[InputHandler] Raw key '${rawInput}' not mapped in current scheme.`);
    }
  }
}
